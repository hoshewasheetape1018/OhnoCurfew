...
namespace PA2
{
    public class Game1 : Game
    {
        ...
        {
            #region Background [NEW]
            bgStage1Texture = Content.Load<Texture2D>("Game Background 15");
            bgStage2Texture = Content.Load<Texture2D>("Game Background 1");

            if (currentStage == 1)
            {
                gameBackground = new GameBackground(bgStage1Texture, new Rectangle(0, 0, Window.ClientBounds.Width, Window.ClientBounds.Height), Color.White);
                LoadStage1Tiles(); // ← ADD THIS
            }
            else if (currentStage == 2)
            {
                gameBackground = new GameBackground(bgStage2Texture, new Rectangle(0, 0, Window.ClientBounds.Width, Window.ClientBounds.Height), Color.White);
                LoadStage2Tiles(); // ← ADD THIS
            }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            int rightEdgeThreshold = Window.ClientBounds.Width - 50;

            if (currentStage == 1 && heroDisplay.X >= rightEdgeThreshold)
            {
                currentStage = 2;
                gameBackground.BgTexture = bgStage2Texture;
        ...
        new Point(800, 400)
            ...
            int totalTiles = manualTilePlatforms.Sum(row => row.Length);
            tiles = new GameTiles[totalTiles];
            ...
            for (int row = 0; row < manualTilePlatforms.Length; row++)
            {
                ...
                for (int i = 0; i < tileDesigns.Length; i++)
                {
                    int tileIndex = tileDesigns[i];
                    Rectangle tileSource = new Rectangle(tileWidth * tileIndex, 0, tileWidth, tileHeight);
                    Rectangle tileDisplay = new Rectangle(posX, posY, tileSize, tileSize);
                    Color tileColor = Color.White;
